...
%   transformParams:   cell array of geometric transform objects
    ...
    for i = 1:numel(alignedImagesGray)
        ...
        indexPairs = matchFeatures(featuresRef, featuresCurr,'Unique', true, 'MaxRatio', 1, 'MatchThreshold', 99.9);
        ...
        if scale < 0.8 || scale > 1.2 || rcond(R) < 1e-6
            warning('❌ Registration rejected for %s. Scale: %.2f, Rotation: %.2f°, rcond: %.2e', ...
    ...
    % Trim unused cells (in case of skipped images)
